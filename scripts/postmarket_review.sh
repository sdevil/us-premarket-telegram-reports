...
python3 "$(pwd)/scripts/build_telegram_compact_review.py" "$OUTFILE" "$COMPACT_OUTFILE"
python3 "$(pwd)/scripts/send_telegram_text_split.py" "$TELEGRAM_TARGET" "$COMPACT_OUTFILE"
printf 'Saved post-market review to %s\n' "$OUTFILE"
printf 'Saved compact Telegram review to %s\n' "$COMPACT_OUTFILE"
